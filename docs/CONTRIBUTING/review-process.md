...
| takumi 軍師 (gpt-5.4 via copilot / codex / opus-max fallback) | 計画・設計の敵対的レビュー | `.takumi/profiles/env.yaml` の preference に従う (`executor.md` 参照) | max 相当 |
...
| 方式 | 結論 |
|---|---|
| critical keyword で**毎回** max 発動 (arm B) | **効果見えず、要 n 増加** — 点推定では arm A と同値。n=8 では「効果なし」を confirm できず inconclusive |
| 全 review で max 発動 (arm C) | **effect size 大、cost 高** — critical/trial で arm A の 3.4 倍、ただし cost 9-13 倍 |
| 選択的発動 (下記 policy) | **ヒューリスティック** — pilot data は直接裏付けていない、運用仮説として採用 |

### 選択的発動 policy (ヒューリスティック)

> [!IMPORTANT]
> 本 policy は pilot data で**直接裏付けられていない**。pilot は「always max vs xhigh」の 2 条件だけを比較し、「large diff 時に発動」「公開 release 時に発動」等の個別 trigger の有効性は未検証。以下は効果量 (arm C の 3.4 倍) と cost (9-13 倍) のトレードオフから導いた**運用仮説**。次の pilot で個別 trigger ごとに効果測定し修正する予定。
