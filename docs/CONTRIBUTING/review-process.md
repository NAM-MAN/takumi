...
Wave 4 pilot (n=24、3 arm 比較、cross-model 含む) の結果を踏まえた発動方針。pilot 結果の詳細は開発者ローカルの記録を参照。

### Pilot 主要結論

| 方式 | 結論 |
|---|---|
| critical keyword で**毎回** max 発動 (arm B) | **採用不可** — corpus で keyword hit が少なく、発動しても recall 向上なし |
| 全 review で max 発動 (arm C) | **採用不可** — recall 4.5 倍 (0.25→1.12) だが cost 13.1 倍、トータル効率悪 |
| 選択的発動 (下記 policy) | **推奨** — 価値の高い局面だけ軍師/max を発動 |
