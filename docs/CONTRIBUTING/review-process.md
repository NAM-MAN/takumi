...
| takumi 軍師 (GPT-5.x via codex / copilot / opus-max fallback、env.yaml driven、auto-fallback 5.5→5.4) | 計画・設計の敵対的レビュー | `.takumi/profiles/env.yaml` の preference に従う (`dispatch/executor.md` の「軍師 routing」+「GPT-5.5 upgrade path」参照) | max 相当 |
| takumi 自己レビュー | 棟梁が職人成果物を見る | 棟梁の直接読み込み | xhigh 既定 |

**棟梁 = Opus 4.8 自身 (Claude Code の main session)。軍師 = 別モデル (GPT-5.x、env.yaml で 5.5/5.4 切替) での cross-model review。**
...
## 自分 (Opus 4.8 main) で直接 review する vs agent に委ねる

[`opus-4-8.md`](opus-4-8.md#subagent-発火条件-抑制優先) に従い、**自分で読める量は自分で review**:
...
**コマンドテンプレート** (tier は `.takumi/profiles/env.yaml` preference.tier に従う、model は preference.model で 5.5/5.4/auto 切替、詳細は `skills/takumi/dispatch/executor.md` 参照):
...
<!-- hardening v2 (2026-05-03): stdin heredoc / `timeout 600s` / 5.5 default / prompt 1.5KB 上限。
  ...
  詳細: `skills/takumi/dispatch/executor.md`「invocation hardening v2」。 -->
```bash
...
## 職人 dispatch の 3-mode routing (実装/修正の担い手)

レビュー対象コードを「誰が実装したか」で読み筋が変わるため、dispatch routing も把握しておく:

- **3-mode capacity-aware routing** (`dispatch/routing-mode.md`): `opus_protect` (全 Sonnet) / `gpt55_priority` (一部 cell を GPT-5.5) / `balanced` (全 Sonnet、4-role 互換)。
- **2026-05-24 の実効 default は `balanced`** (codex degraded で信頼できる GPT-5.5 transport 不在、Claude-only)。機構は保持し transport 回復で自動復帰 (可逆)。
- したがって現状のレビューは「Sonnet 実装 → 棟梁 gate check」が主経路。GPT-5.5 実装物が混ざる場合は cell mapping (T1/T3/T4/T8/T9) を確認。
