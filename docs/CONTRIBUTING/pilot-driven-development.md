...
<!-- hardening v2 (2026-05-03): stdin heredoc / `timeout 600s` / 5.5 default。
  ファイル本文は呼出側で埋込み、codex に「読め」命令で hang trigger を引かない (詳細: `skills/takumi/dispatch/executor.md`「invocation hardening v2」)。 -->
```bash
PROMPT_FILE=$(mktemp)
cat > "$PROMPT_FILE" <<EOF
...
### 6-a. 直接反映の前例 — layout-primitives (2026-05-25)

§6 の「skill 直接反映への切替」を実際に行使した前例。design-prevention pilot iteration 1 が RETREAT (2026-04-24) した後、prevention-first の中核アイデア (primitive / token 抽象層) を **pilot で検証する代わりに skill へ直接反映**した。

- **成果物**: `skills/takumi/design/layout-primitives.md` (構成的予防の一次層、2026-05-25 追加)
- **帰結した設計**: **二層モデル** — `layout-primitives.md` (L-construct = 崩れない primitive 合成で生成空間を狭める一次予防) を一次層、`design/l7-invariant.md` (L-detect = 構造化レンダー監査) を保険網に再定義
- **caveat (重要)**: 本層は pilot 検証も marginal uplift 測定も経ていない「直接反映」である。効果の定量検証は v3 pilot (§11 の entry criteria 充足後) の対象として保留。直接反映は「結論を即座に skill 化する」ことであって「効果が実証済み」を意味しない

含意: §5 の「採用された仕様のみ skill に置く」は、pilot 採用に加えて **RETREAT 後の直接反映 (規範的仕様)** も含む。役割分担は — 出所と未検証性は **本書 (dev 向け) に記録**、skill ファイル側には layer 位置付け (一次予防 / 保険網) のみ明記する。これにより §5「配布物に実験痕跡を残さない」を保ちつつ、失敗試行の残骸 (drafts 行き) と規範的仕様を区別できる。
