# opus-4-8.md — Opus 4.8 運用方針

> [!NOTE]
> **このファイルは takumi skill を開発・フォークする人向け** です。`/takumi` を使いたいだけの方は [README.md](../../README.md) を参照してください。

2026 年 6 月時点の Claude Opus 4.8 (Claude Code 経由) の公式推奨と本リポジトリの適用方針。根拠は [Best practices for using Claude Opus 4.7 with Claude Code](https://claude.com/blog/best-practices-for-using-claude-opus-4-7-with-claude-code) (Anthropic 公式 blog、4.7 策定時点)。

4.7→4.8 の固有の挙動変化は公式ガイド未公開のため、本書の effort/subagent/adaptive thinking 方針は 4.7 ベストプラクティスを 4.8 でも踏襲する (新ガイド公開時に見直す)。

> [!IMPORTANT]
> **2026-06-25 既定モデル更新 — Opus 4.8 に戻す (Fable demote、現行)**: Fable 5 が **preview status** になり、無料枠 (~6/22) 終了で **Opus の 2× 価格**に。無人実行 (Wave loop) の既定に preview モデルを置く根拠が消失したため、`default_mode` を **`balanced` に revert**・棟梁を **`claude --model opus`** に戻した。軍師 (codex gpt-5.5、cross-family) 裁定 = **GO-with-fix** (demote 妥当 / balanced は最小リスク退避先 / fable は opt-in 保持 / 再昇格 gate を明文化)。Fable は manual_override で取得可、再昇格条件は env.yaml `routing.fable_repromotion_gate` (preview→GA / 価格≤Opus / cross-family judge + 複数repo・複数trial・長時間自律 task の再 pilot / cost-adjusted quality > balanced)。裁定全文 `.takumi/notepads/fable-demote/oracle-verdict.md`。
>
> <details><summary>2026-06-11 の Fable 既定化 (2026-06-25 で revert 済、履歴)</summary>
>
> `fable5-routing-pilot` (40-task × 3-arm blind paired judge) で Claude Fable 5 が judged code quality で Opus 4.8 を ~66% / Sonnet 4.6 を ~72% 上回り (機能性 median 5.0 vs 4.0)、objective gate 非劣 → user 判断で棟梁・職人とも Fable を既定に格上げ。ただし機械 verdict は cost 根拠で HOLD、**Fable 本領の長時間 autonomous + self-verification は本 corpus で未測定** だった。preview 化 + 価格確定で上記 2026-06-25 に opt-in へ降格。詳細 `.takumi/pilot/fable5-2026-06-11/result-summary.md`。
> </details>
>
> 本書の effort/subagent/thinking 方針は Opus 4.8 / Fable いずれでもそのまま適用 (xhigh 既定)。routing は `skills/takumi/dispatch/routing-mode.md`。

---

## 4.8 で確実に新しい点

1. **1M context window** (`claude-opus-4-8[1m]`) — 希釈余地は増えるが希釈対策 (thin orchestrator + per-Wave re-anchor + 外部権威状態) は窓サイズに非依存で不変
2. **Fast mode 対応** (Opus 4.8/4.7/4.6、`/fast`、小さいモデルに落ちない)

---

## 変わった 3 点 (4.6 → 4.7、4.8 でも踏襲)
...
model 軸は `.takumi/profiles/env.yaml` の `preference.model` (`auto` / `gpt-5.5` / `gpt-5.4`) で制御。auto は tier 内 highest available を選び、5.5 → 5.4 fallback 時は stderr に 1 行通知 + telemetry 記録。詳細は `skills/takumi/dispatch/executor.md` の「GPT-5.5 upgrade path」。
...
詳細 (exact 呼出構文 / detection / quota rotation) は `skills/takumi/dispatch/executor.md` の「軍師 routing (3-tier + quota rotation)」節を参照。
...
Opus 4.8 は「簡単なら短く、分析なら長く」を勝手に判断するが、外すことがある。以下を明示する習慣:
...
## 5 ロール (takumi skill 内部) との関係

takumi skill は 5 ロール体制 (棟梁/軍師/職人(Sonnet)/職人(GPT-5.5)/斥候) を定義していますが、これは **takumi が解くタスクの中での委譲設計** です。本 `opus-4-8.md` の subagent 抑制ルールとは**直交** (別次元)。

- takumi 利用者が `/takumi` で依頼 → 棟梁が必要に応じて軍師/職人/斥候を spawn (これは takumi の設計)
- 本リポジトリの**開発者**が takumi を改善する → 開発者 (Claude Code) が subagent を spawn するかは Opus 4.8 ルールに従う
...
- 外部: [Anthropic 公式: Opus 4.7 best practices (4.8 踏襲)](https://claude.com/blog/best-practices-for-using-claude-opus-4-7-with-claude-code)
