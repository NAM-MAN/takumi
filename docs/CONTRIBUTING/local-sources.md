...
- `git remote -v` が `toishi2/takumi` を示す — upstream 判定の参考、fork では異なる remote を持つので必須条件ではない
...
| ユーザーの `~/.claude/agents/*.md` を前提に設計 | 本リポジトリは agent 定義を持たないのに agent 固有 API を仮定 | `skills/takumi/SKILL.md` の 5 ロール定義 (棟梁/軍師/職人(Sonnet)/職人(GPT-5.5)/斥候) を参照 |
