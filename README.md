...
| ロール | モデル | 担当 | ポイント |
|---|---|---|---|
| **棟梁** (とうりょう) | Claude Opus 4.7 | 全体設計・計画作成・会話の中心 | 小規模は自分で直接実装 (subagent 抑制) |
...
| `profiles/verify/` `profiles/design/` | 候補 | チーム共通の基準を共有したい場合 (strict-refactoring 用 `profiles/refactor/` は将来拡張予定、現状は未 bootstrap) |
