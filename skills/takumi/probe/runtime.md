...
## 発見者並列起動 (discovery orchestrator)
...
発見者定義は `probe/roles/*.md` に分離予定 (保守性優先)。`probe/discover.md` は「観点語 → 発見者 ID」のマッピング。harness が揃う behavioral surface では発見ラダー **L6 = 巡視** (`../junshi/`) まで登る。
...
| 条件 | 判定方法 | アクション |
|------|---------|-----------|
| 完了 (限界効用) | バックログ空 **かつ 発見ラダー (`discover.md` L0-L6) を L6 (harness 不在なら L5/L4) まで登り切って新レンズでも新規発見が出ない** + 自動点検で追加なし | 完了処理へ |
| 中断 | ユーザーが「止めて」 | 状態保存して終了 |
| 継続保存 | コンテキスト残量20% | resume.md 生成して終了 |

> [!IMPORTANT]
> **収束は「空集合」でなく「限界効用」で判定**。現レベルが新規発見ゼロになっても即停止しない — **次レベル (L1→L2→L3→L4→L5→L6、L5/L6 は条件付) のレンズを起動**し、レンズを敵対的に強くする。**新レンズ (観点生成器自体) が飽和したときのみ収束**。固定観点1巡の「新規発見なしで停止」は family を取り逃すため廃止 (`discover.md` 発見ラダー参照)。
