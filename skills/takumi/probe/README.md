...
<p align="center">
  <img src="https://img.shields.io/badge/skill-probe-7C3AED" alt="Skill: probe" />
  <img src="https://img.shields.io/badge/mode-discover%E2%86%92fix-success" alt="Mode: discover to fix" />
  <img src="https://img.shields.io/badge/runtime-auto--loop-orange" alt="Runtime: auto-loop" />
</p>
...
```mermaid
flowchart LR
    U[ユーザー<br/>/probe &lt;観点&gt;] --> D[発見<br/>discover.md]
    D --> T[選別<br/>triage.md<br/>MECE + ICE + Oracle]
    T --> P[計画<br/>plan skill<br/>backlog-mode]
    P --> E[実行<br/>/exec + 自動点検]
    E -->|新たな発見| D
...
> [!NOTE]
> ここから下は `/probe` を実行した AI エージェントが Read する運用仕様です。Phase 0 委譲ガード、Phase 0a 初期化、Phase 1-4、再開、状態確認、終了条件、完了処理、制約を記述しています。内容は runtime で必要なため要約しません。
...
<details>
<summary><strong>委譲の起動条件 / 委譲手順 / Agent 委譲プロンプト本体（JSON スキーマ含む）</strong></summary>
...
<details>
<summary><strong>再開（<code>/probe continue</code>）</strong></summary>
...
<details>
<summary><strong>状態確認（<code>/probe status</code>）</strong></summary>
