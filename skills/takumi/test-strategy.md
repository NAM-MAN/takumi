...
derived_from: ["I4", "T3"]  # optional、TopContract 由来 (contract-spine.md)。I3/I4 由来は property/model に寄与
context:
  layer: "domain" | "ui" | "api" | "data"   # surface の 6 軸タグからも導出可
  risk: "low" | "standard" | "critical"
  project_mode: "ui" | "mixed" | "backend"
```

> `derived_from` に I3 (状態不変) / I4 (遷移合法性) が含まれれば property / model archetype に寄与 (ConsistencyMatrix M8: AC ↔ test layer の整合)。A/B/C の導出ロジック自体は不変。
...
| `data_access.optimistic == true` (DDP) | layers に **L3 Differential** を付加。property: **`reconcile(applyOptimistic(s, m)) == serverOnly(s, m)`** (楽観経路と server-only 経路が同状態に収束)。失敗時 rollback で `s` に戻ることも property 化。`data-access-protocol.md` |
...
| metamorphic | 画像 / ML / LLM / 分類 / 変換 / 回転しても / 同一クラス / 正解が書けない / オラクル不在 / metamorphic / 楽観 / optimistic / 即時反映 / 収束 (楽観↔server-only 差分は metamorphic+differential、`data-access-protocol.md`) |
...
> [!NOTE]
> **軍師モデル表記**: 以下の `-m gpt-5.5` は default (env.yaml `preference.model: auto`/`gpt-5.5`)。`gpt-5.4` 強制は env.yaml で設定可、auto-fallback rule は `gunshi-invocation.md`「GPT-5.5 upgrade path」参照。stdin heredoc / `tk_timeout 600` / prompt 1.5KB 上限。

A / B で決まらなければ 軍師 (GPT-5.x) に 1 行 archetype 名だけ返させる:

```bash
tk_timeout 600 codex exec -m gpt-5.5 -s read-only --skip-git-repo-check -C "$(pwd)" - <<'PROMPT' 2>&1 | tail -5
...
> **AC 品質 advisory (§C、同 batch で実行)**: 上記 resolve と同じ 1 batch で、各 AC の gwt 観測可能性 (曖昧語 / oracle 不在 / scope 未定義) と TopContract trace を **LLM advisory** で点検し、低品質 AC を author に flag する。**hard gate にしない** (機械的禁止語彙は subtle ケースを捕捉できず、検出力が LLM 依存になるため — 詳細 `contract-spine.md` validation #6)。authoring 時 1 回・1 batch call なので per-Wave overhead はゼロ。

---

## 月次保守ルール (陳腐化防止)

導出ルールは 3-6 か月で陳腐化する。毎月以下を実施:
