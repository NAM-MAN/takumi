...
本 skill (`SKILL.md`) から参照される補助ドキュメント。strict-refactoring の Tier と verify skill の archetype は**別軸**だが、新ワークフローでは両者に対応関係がある。「新ワークフローで絶対に壊すべきでない contract」もここに集約。
...
| `../telemetry/telemetry-spec.md` | `verify_contract_satisfied` event |
