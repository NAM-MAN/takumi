...
**骨格**: `SHARPEN > PRUNE > ADD, gate by survived/no-cov non-regression`。生成時の冗長 (B1-B5: 過剰防御/再記述/早すぎる抽象/what コメント/過剰 error 包み) を断つ姉妹規律は **`ai-brevity.md`** (refactor 時の SMD に対し micro/token 経済、非重複)。

#### 17. 宣言的デフォルト (collection transform)

collection 変換は **宣言的 default**。`for` は 4 exception のいずれかに該当するときだけ残す。Rule 16 (マクロ) が「何を削るか」なら Rule 17 は「どう処理するか」。詳細と実測傾向は **`immutable-first.md` §3**。
...
**実測傾向**: 1 unit あたり 3 refactor 程度、LoC 2-5% 減、survived 不変、可読性優位 exception が 2-3 箇所に 1 回発動する頻度。

**骨格**: `for → map/filter/flatMap/reduce (default); for only if 副作用・中断・perf・可読性`

#### 18. Immutable Construction (immutable 構築)

構築時に mutation を避け、**literal / spread / filter で直接構築**する。Rule 17 (処理) / Rule 20 (束縛) と同じ "avoid mutation" 原則の構築レイヤー表現。詳細と実測傾向は **`immutable-first.md` §2**。
...
**Rule-of-N DRY との相性**: 同一 pattern を 3-4+ 箇所で construct する場合、helper 化で集約すると **survived mutant が減る**副作用あり (重複 defensive が 1 箇所に集約 → テスト密度が N 倍に上がるため)。
...
**実測傾向**: 1 unit あたり LoC 10-15% 減、**survived 30-50% 減 (品質改善)**、mutation score +2-5pt。
...
操作の carrier は **層ごとの既定 (prior) → 4 力 (裁定) → 構文** の 3 段で決める (実行カード・境界ケース・移行方針は **`behavior-carrier.md` §0**)。**層既定**: domain=method/SVO (不変条件所有が常態の earned default)、application/変換=function、infra=class-DI or closure factory を層内一貫、UI=hook。global な function-first も class-first も誤り (前者は domain に over-claim、後者は god-object 化)。層が未分化なら prior を置かず下の 4 力で直接裁定。判定軸は state/identity ではなくこの 4 力。

| # | 力 (逸脱理由) | 既定 carrier (構文は層の一貫性で別途選ぶ) |
|---|---|---|
| **EXC1** | **invariant-owner** — Y が invariant・状態遷移を所有 (persistence/transport/I/O 除外) | `y.doX()` aggregate method (Rule 19) |
| **EXC2** | **shared-resource** — shared stable dependency set を複数 method が共有、deps が call ごとに不変 (個数でなく reuse/cohesion) | class + constructor DI **または** closure factory (SC6) |
| **EXC3** | **polymorphism** — method ごとに**ロジック・失敗モード・契約が異なる** | interface + 複数 impl |
| **EXC4** | **framework-lifecycle** — UI state + framework lifecycle 結合 | React hook |

**force と carrier 構文は別軸** (詳細 §0): 4 力は逸脱の要否、構文 (class / closure factory / struct+impl) は層の一貫性で決める。`class` は構文でなく「不変条件を所有する名前付き型」の意。
