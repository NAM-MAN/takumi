...
`rules-heuristics.md` の **Rule 19 (Subject Owns Verbs) / Rule 21 (Behavior Carrier Selection) / Rule 17-D (dispatch 判断フロー)** を実運用に落とす recipe。`immutable-first.md` が **how to write statements** なら、本 md は **how to structure operations** (操作の carrier を §0 の「層の既定 → 4 力 → 構文」の 3 段で決める)。

---

## 0. 実行ルール — carrier 選択カード (これだけで 9 割決まる)

新規コードの操作 `do X to Y` の carrier は **3 段**で決める。§2 の決定木を毎回辿らず、まずここで pattern-match し、borderline のみ §2-3 を引く。

### 段1 — 既定は層ごとに変える (prior)。各層の既定 = その層の最頻失敗モードの解毒剤

| 層 | 最頻失敗モード | 既定 carrier |
|---|---|---|
| **domain** (aggregate/entity, 不変条件 rich) | 不変条件が散る | **method / SVO** (`cart.addItem(x)`, `order.validate()`) — invariant-owner が常態なので earned default |
| **application / 変換** (usecase, mapper, orchestration) | 意味の薄い object に処理が吸われる | **function** (`submitOrder(input)`, `transfer(from, to, amount)`, `toOrderDto(order)`) |
| **infra** (repository, client, resource) | lifecycle/依存が見えなくなる | **class + DI または closure factory、層内で一貫** |
| **UI** (React) | state/lifecycle が散る | **hook** (`useCart()`) |

> global な「function-first」も「class-first」も誤り。前者は domain に over-claim (不変条件遷移を毎回関数化する摩擦)、後者は `from.transferTo(to, amount)` のような恣意的主語と god-object 化を生む。**既定は層が決める。**
> **層が未分化な小規模アプリ**で domain/app の線が引けない時は prior を置かず、段2 の 4 力で直接裁定する。

### 段2 — 4 力 = 個別ケースの裁定者 (prior を上書きする ground truth)

層の既定はあくまで prior。実際の carrier は、その操作が下の 4 力のどれを名指しできるかで確定する。

| 力 | 意味 | 落とし所 |
|---|---|---|
| **invariant-owner** | Y が X の不変条件・状態遷移を所有 (persistence/transport/I/O は除外) | `Y.doX()` aggregate method (Rule 19) |
| **shared-resource** | stable な共有依存集合を複数 method が共有、deps が call ごとに不変 | deps を 1 度束ねる (constructor DI 等) |
| **polymorphism** | 実装ごとにロジック・失敗モード・契約が異なる | interface + 複数 impl |
| **framework-lifecycle** | UI state + framework lifecycle (React render 等) に結合 | hook |

どの力も名指しできなければ **`export function doX(y)`**。**判定軸は state でも identity でもない** — `useState` も closure も stateful なので「state を持つ→class」は誤り、UUID を持つ値オブジェクトも data なので「id を持つ→class」も誤爆。carrier を決めるのは力であって state/id の有無ではない。

> **SVO は earn されるもの**: `order.validate()` が良いのは英語的だからでなく Order が不変条件を所有するから。`order.exportCsv()` / `order.sendEmail()` は発見性ではなく**責務の吸引事故**。`order.` 補完の discoverability は**責務が閉じた domain aggregate でだけ利点**、開いた carrier では god-object の導火線。

### 段3 — carrier 構文 (class / closure factory / struct+impl) は層の一貫性で別途選ぶ

力が逸脱を正当化しても **構文は強制されない**。`shared-resource` は `class + constructor DI` でも `const repo = makeOrderRepo(db)` でも等価 — **同一 layer 内で一貫**していればよい (関数派 codebase は後者が自然、SC6)。「class」は構文でなく **「不変条件を所有する名前付き型」** の意で、Rust `struct + impl` / Go `struct + method` でも同じ力を表現できる。

つまり **①層で prior → ②4 力で裁定 → ③層の一貫性で構文**。§2 の Q1-Q5 はこの手続きの詳細版・境界ケース集で、迷った時だけ参照する。
