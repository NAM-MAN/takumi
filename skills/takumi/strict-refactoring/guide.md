...
## 保全ゲート (refactor 前の必須判定)

> [!IMPORTANT]
> profile を選ぶ**前**に「そもそも refactor すべきか」を判定する。既存コードが **battle-tested** なら、clean pattern への retrofit は net-negative になりうる (積層した修正を silent に regression させる)。これは `legacy-touchable` (緩く触る) のさらに上流の「触らない」判定。新 profile は増やさない (制約遵守) — gate の出力として `preserve` を返すだけ。

**保全シグナル** (高いほど「触らない」側):
- コード内に番号付きバグ修正コメント (F1 / F23 / P2 等) や「〜防止」注記が積層している
- 既存テストが厚い (特に PBT / integration が既に存在)
- 統合が深い (多数の呼出元、外部プロトコル / SDK 依存、realtime 等)
- 失敗影響が critical (決済 / 認証 / テナント分離)

**判定**:
- シグナル高 → profile を当てず **`preserve`** を出力。refactor せず、周囲に **テスト足場 (fast-tier fake / Fake≡Real contract / characterization test)** を足すに留める。clean pattern は greenfield 側に置く
- シグナル低 (新規 / 浅い) → 通常の profile 選定へ

> 経験的裏付け: 堅牢な Stripe webhook の純reducer 化 / Realtime 依存下の RLS 配線変更 / 既に純関数+PBT 化済の認可層 — いずれも retrofit より保全が正解だった。**「全部を綺麗な型に書き換える」のでなく「どこを残すか」の判断こそ methodology の核**。greenfield には clean pattern、既存堅牢コードには保全 + テスト基盤の追加、という棲み分け。
...
> [!IMPORTANT]
> Tier B の `actionPreconditions` を verify が fc.commands で再利用することで、**production と test が同じ実体で drift しない**。これが新ワークフローで**絶対に壊すべきでない contract**。
...
詳細 schema は `../telemetry/telemetry-spec.md` と `telemetry-schema.md` (後日追加)。
