...
> [!NOTE]
> **軍師モデル表記**: 例文は `gpt-5.5` default (env.yaml `preference.model: auto`/`gpt-5.5`)。`gpt-5.4` 強制は env.yaml で設定可、auto-fallback rule + hardening v2 (stdin heredoc / `tk_timeout 600` / prompt 1.5KB 上限) は `gunshi-invocation.md`「GPT-5.5 upgrade path」+「invocation hardening v2」参照。
