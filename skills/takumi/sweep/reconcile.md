...
## syntheses 本文
$(cat .takumi/sprints/{日付}/syntheses.md)
EOF
tk_timeout 600 codex exec -m gpt-5.5 -s read-only --skip-git-repo-check -C "$(pwd)" - < "$PROMPT_FILE" 2>&1 | tail -200
```

> syntheses が 1.5KB を超える場合は提案を 5 件ずつバッチに分割するか、subagent (Sonnet via Agent tool) Tier 2 で直接 dispatch する。timeout 超過時も同 fallback。
