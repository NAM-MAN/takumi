...
<p align="center">
  <img src="https://img.shields.io/badge/skill-sweep-7C3AED" alt="Skill">
  <img src="https://img.shields.io/badge/dimensions-D1--D8-success" alt="Dimensions">
  <img src="https://img.shields.io/badge/strategy-parallel+reconcile-orange" alt="Strategy">
</p>
...
```mermaid
flowchart TD
    Start(["/sweep 起動"]) --> Guard{Phase 0 ガード}
    Guard -->|in_progress| Skip(["即終了"])
    Guard -->|paused| Resume["/sweep continue"]
...
stateDiagram-v2
    ...
    Paused --> ResumeFlow: /sweep continue として再開
...
> [!NOTE]
> ここから下は `/sweep` を実行した AI エージェントが読む仕様セクションです。人間の読み物ではなく、AI runtime が参照する運用仕様なので、Phase 0 〜 5 の手順・契約・禁止事項を省略・改変せずそのまま保持しています。
...
> [!IMPORTANT]
> `.takumi/state.json` を読み、前回の状態で分岐する。`in_progress` なら即終了、`paused` なら `/sweep continue` として再開、それ以外は続行。
