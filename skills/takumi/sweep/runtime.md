...
## backlog 連携 (sweep 出力の昇格、`SKILL.md` Step 0e)

sweep 完了時 (統合済み発見リスト確定後)、`OfferPolicy.shouldOffer('sweep_complete')` を経由して提案発火。`mode == enabled` なら各発見を `.takumi/backlog/open/BL-###-{slug}.md` に自動昇格 (source: sweep)。`external` / `unset` / `deferred` 期限内では従来通り `sprints/{date}/` フローを継続 (silent 違反防止)。詳細: `backlog-mode.md` + `backlog/offer-policy.md`。
