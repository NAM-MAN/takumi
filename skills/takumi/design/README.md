...
さらに一歩手前で、**そもそも崩れない部品からしか画面を組めないようにします**。レイアウトは「崩れない実装パターン (primitive)」の有限集合を入れ子に合成し、AI に自由な CSS を書かせません。崩れる書き方を「書けるけど後で直す」のではなく「書けなくする」という発想です。これが一次の予防層で、下記の検証 3 層は、それでも残る境界条件・ブラウザ差・未知コンテンツを捕まえる**保険網**として働きます (詳細は AI 向け仕様の `layout-primitives.md`)。
...
### 7. プロが組んだような craft (「壊れない」の先へ)

ここまでの 6 視点は「UI が**壊れない**」(overflow ゼロ・コントラスト・クリック範囲) を守ります。これは品質の**床**であって**天井**ではありません。L7 を全通過しても、凡庸 (AI っぽい) な画面にはなり得ます。

design スキルには第 4 の **Craft Constraint 層**があります。美しさを"生成"するのではなく、**凡庸さを構成的に減らす制約**を置きます (`craft-layer.md` / `craft-tokens.md`)。

- **色は OKLCH の階調 (ramp) と役割 (Radix 12-step) で導出** — ad-hoc な hex を手で明暗しない
- **影は多層 (elevation)・光源一貫・dark は明度で表現** — 単一 box-shadow の安っぽさを避ける
- **日本語組版を first-class に** (`jp-typography.md`) — 行間 (unitless)・文字詰め (`palt`)・禁則・和欧混植・measure・Latin-first font stack。プロの日本人デザイナーが Figma で詰める所を CSS で再現
- **AI っぽさの blocklist** — Inter 既定 / purple-on-white / 単一影 / pure gray / 面と罫線の増殖 を soft rule で抑制
- **参考サイトは「Linear っぽく」でなく原理 (density / contrast / motion / surface) に分解**して seed
- 効果測定は build を止めない **offline 観測装置** (taste oracle) で、頻出する欠陥だけを決定論ルールに昇格 (スキルが使うほど強くなる)

> [!IMPORTANT]
> craft は「壊れない」層を**破れません**。優先順位は a11y/安全 > レスポンシブ > 情報構造 > craft。演出的なはみ出し (decorative bleed) は許しても、文字・操作要素のはみ出し (functional overflow) は許しません。
...
| **Craft Constraint 層** | 「壊れない」の先で「凡庸でない (プロが組んだような)」を狙う第 4 層 |
| **OKLCH** | 知覚的に均一な色空間。階調 (ramp) や明暗を破綻なく作れる (HSL より優秀) |
| **Elevation** | 影や明度で表現する「面の浮き上がり」。多層 shadow + 光源一貫が pro |
| **palt / 和欧混植** | 和文の比例詰め (OpenType) と、日本語と英数字を美しく混ぜる組版 |
| **locale_profile** | `ja-JP` / `en-US` / `mixed`。言語ごとの組版規範を切替える設定 |
