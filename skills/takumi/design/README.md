...
> [!NOTE]
> ここから下は `/design` を実行した AI エージェントが読む仕様セクションです。人間向けの LP パートとは役割が違うため、運用上の不変条件 (Phase 詳細・必須入力 4 項目・L7 hard gate・verify 連携・制約・導入チェックリスト) をすべて保持しています。削除・要約しないでください。
