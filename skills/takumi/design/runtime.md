...
3. **凡庸さを構成的に減らす** — craft primitive (OKLCH ramp / 多層 elevation / 日本語組版 /
   principle dictionary) を決定論で固定し、「壊れていない」だけでなく「プロが組んだように見える」
   を狙う。taste を seed せず **craft を seed する** (`craft-layer.md`、第 4 層)。
...
## L-construct: 崩れない部品から組む (構成的予防、一次層)

UI 崩れは**検出して直す**前に**構成的に予防**する。AI に自由 CSS を書かせ後段で直すのは負け筋なので、画面骨格は崩れない primitive の有限集合 (10 種) を入れ子合成し、生成空間そのものを狭める。

- **10 primitive の nest 合成のみ**で骨格を組む。`position:absolute` と layout margin は escape hatch 宣言外で lint 禁止。
- 各 primitive に**防御 CSS を焼き込む** (flex/grid 子 `min-width:0` / text `overflow-wrap:anywhere` / media `max-width:100%` / grid `minmax(0,1fr)` / scroll ownership 単一化)。これで入れ子の非 overflow 閉包性が成立する。
- **StylePassPolicy** で layout-then-style 2-pass を機械強制。Phase A (骨格) は layout utility 専有、Phase B (styling) は skin allowlist のみで、layout utility 混入を lint block する。
- 素 primitive (`<input>` + 自前 absolute icon 等) を AI に渡さず、状態を variant 化した合成 component のみ公開する。

この一次予防が健全なら、次節 L7 の hard gate は稀にしか発火しない。**L-construct (予防) が一次、L7 (検出) は保険網**という二層関係。詳細 (10 primitive 表 / 防御 CSS / intent→primitive routing / lint 禁止 6 rule / surface 別 strictness + escape hatch) は **`layout-primitives.md`** を読む。

---

## L7 Layout Invariant (3 層、概要)

L-construct で防ぎきれない残余 (境界条件・ブラウザ差・未知コンテンツ・escape hatch 領域) を実測で捕捉する**保険網**。画面生成後の検証は 3 層に分ける。**hard gate は最小限**に留め、soft / lint に降ろす。
...
## Craft Constraint 層: 凡庸でない (第 4 層、概要)

L-construct (崩れない) / L7 (壊れていない検出) / seeded token (一貫性) と**直交**する第 4 層。「L7 全通過でも凡庸 (AI っぽい)」を防ぐため、taste でなく **craft primitive を決定論で固定**する:

- **色**: OKLCH 50–950 ramp + Radix 12-step semantic + APCA Lc 目標 (ad-hoc hex 廃止)
- **elevation / radius / spacing / type scale / motion**: 導出規則で固定 (`craft-tokens.md`)
- **日本語組版**: `locale_profile` (既定 `ja-JP`) + 禁則 / palt / 和欧混植 / Latin-first stack (`jp-typography.md`)
- **principle dictionary**: ref を brand 名でなく原理 enum で seed
- **AI-slop blocklist** + **craft preflight rubric** (`craft-layer.md`)

craft は L7 を破れない (優先順位: **a11y > レスポンシブ > 情報構造 > craft**、overflow は functional/decorative で分離)。craft の効果測定は **offline 観測装置** (`taste-oracle.md`、pilot 採否保留) であって gate にしない。詳細は **`craft-layer.md`** を読む。
...
| `layout-primitives.md` (同ディレクトリ) | UI 崩れの構成的予防 (一次層、10 primitive + 防御 CSS + StylePassPolicy + lint) |
| `l7-invariant.md` (同ディレクトリ) | L7 hard/soft/lint の検出・昇格・PBT テンプレ (L-construct の保険網) |
| `craft-layer.md` (同ディレクトリ) | Craft Constraint 層 doctrine (principle dictionary / AI-slop blocklist / craft preflight rubric / 矛盾調停) |
| `craft-tokens.md` (同ディレクトリ) | craft token 決定論導出 (OKLCH ramp / elevation / radius / spacing / type / motion) |
| `jp-typography.md` (同ディレクトリ) | locale_profile + 日本語プロ typography base layer |
...
| `../telemetry/telemetry-spec.md` | `layout_checked` event の emit |
