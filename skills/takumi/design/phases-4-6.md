...
> [!NOTE]
> easing curve / duration / 全 interactive state の **craft 規範** (調整 cubic-bezier、hover/active/focus-visible/disabled の別個設計、1 orchestrated entrance > 常時微動) は `craft-tokens.md` §6 を準拠。下表はその適用例。motion 量は profile の `motion` 原理 (minimal/subtle/expressive、`craft-layer.md`) に整合させる。
...
> [!NOTE]
> ASCII 骨格の各領域は `layout-primitives.md` の 10 primitive のどれかに対応させる (例: header/sidebar/content 分割 = Sidebar、filter 群 = Cluster、table = ScrollArea でラップ、カード一覧 = Grid)。実装段ではその primitive を nest 合成するだけで、自由 positioning を書かない (構成的予防の一次層)。
...
さらに **craft 観点** (視覚階層 / 型スケール / 余白リズム / 色調和 / elevation / state design / iconography / alignment / density / motion restraint / 日本語の行長・行高) は `craft-layer.md` §6 の **craft preflight rubric** (anchored level 0=凡庸 / 3=pro) を self-review に併用する。素人視点 (体感の不便) と craft 観点 (作り込み) は別軸として両方確認。taste の総合採点や「プロっぽさ」「AI っぽさ」は gate にしない (offline 観測装置 `taste-oracle.md` に委ねる)。
