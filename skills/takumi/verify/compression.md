...
## 9. 時間軸 ledger + cost-aware PRUNE (B-stack)

詳細 recipe は **`compression-vitals.md`** に分離 (削除 precision は実 repo + 独立 oracle での標本検証前提、同一スクリプトの self-check は循環論証):

- **B1 寿命 ledger** (`.takumi/verify-loop/test-vitals.jsonl`): 各 test の `unique_kills / spec_density / cost_ms_p95 / kill_count_30d / subsumed_by` を append-only で記録
- **B2 cost-aware PRUNE 順序**: zero_contribution → subsumed → cost desc → spec_density asc → mutual subsumption の tie-break (spec_density 高 → 「べき」naming → cost 安 → test_id 小)
- **mutual subsumption tie-break** ルールで現状の曖昧さを解消
- 月次 30 日 window で `kill_count_30d` を再計算、3 ヶ月 unique_kills=0 を PRUNE escalate

→ L4 primary tier project のみ適用、運用は 3 ヶ月 ledger 前提。
