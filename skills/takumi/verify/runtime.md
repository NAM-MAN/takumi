...
6. **1 unit = 1 test file = 仕様書** — `.pbt.test.ts` / `.mutation.test.ts` / `.metamorphic.test.ts` 等の機構別ファイル分割を**禁止**。`it('{Subject} は {input} に対して {output} を返すべき')` の中で PBT / metamorphic / commands を選ぶ。詳細 → `spec-tests.md` (strict-refactoring Rule 14 を継承)

> [!IMPORTANT]
> 原則 6 は「鋭いテスト専用ファイル」の構造的問題を根絶するための**最重要不変条件**。test 名は機構 (PBT, P1, mutation-killer) ではなく仕様 ({Subject} は {input} に対して {output} を返すべき) を語る。
