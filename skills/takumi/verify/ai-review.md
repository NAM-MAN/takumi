...
> [!NOTE]
> **軍師モデル表記について**: 本ファイルの軍師呼出例 (`-m gpt-5.4` / `--model gpt-5.4`) は guaranteed baseline。`.takumi/profiles/env.yaml` の `preference.model: auto` 時、ChatGPT Plus user / Copilot Pro+ user の runtime は **gpt-5.5** が選ばれる (詳細: `gunshi-invocation.md`「GPT-5.5 upgrade path」)。「軍師 (gpt-5.4)」「GPT-5.4 (軍師)」表記は文書上の baseline 表現で、実 runtime の model は env.yaml driven。
...
# stdin heredoc + tk_timeout 600 overall hard cap (long-thinking レビューを切らないため、hang 確実に kill する値) + 5.5 default
PROMPT_FILE=$(mktemp)
cat > "$PROMPT_FILE" <<EOF
...
tk_timeout 600 codex exec -m gpt-5.5 -s read-only --skip-git-repo-check -C "$(pwd)" - < "$PROMPT_FILE" > .verify-review.json
EXIT=$?
rm -f "$PROMPT_FILE"
  ...
  oracle-review:
    ...
    steps:
      ...
      - name: 軍師 review
        ...
        run: |
          # stdin heredoc + tk_timeout 600 overall hard cap + 5.5 default
          ...
          $DIFF
          EOF
          tk_timeout 600 codex exec -m gpt-5.5 -s read-only --skip-git-repo-check -C "$(pwd)" - < "$PROMPT_FILE" > review.json
          EXIT=$?
          rm -f "$PROMPT_FILE"
