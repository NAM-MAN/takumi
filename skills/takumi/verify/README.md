...
![Layers](https://img.shields.io/badge/layers-L0--L6-success)
...
| **cross-model audit** | 軍師 (GPT-5.x; baseline gpt-5.4、env.yaml auto で Plus user は gpt-5.5、詳細: `~/skills/takumi/gunshi-invocation.md`「GPT-5.5 upgrade path」) による AI 書き PR レビュー | Q4 drift の **遅延検出** |
...
```mermaid
flowchart LR
    A[verify] --> L0[L0 型規律 最小にして最強]
    A --> L1[L1 Property-Based]
    A --> L2[L2 Component Test]
    A --> L3[L3 Model-based + Differential]
    A --> L4[L4 Mutation]
    A --> L5[L5 Smoke E2E]
    A --> L6[L6 AI Review]
    L0 -.illegal state を型で消す.-> OUT[カバレッジでは見えない品質]
    L1 -.pure function.-> OUT
...
### 0. 型で実行時例外を消します (L0-type、最下層 = 最強の最小テスト)

> [!IMPORTANT]
> 6 つの視点はすべて **実行時** の手法です。その**下に L0-type** を置きます。正しい型は**コンパイル時に成立する証明**で、無数の runtime example より安く強い。`any` / 不要な `undefined` を削り、illegal state を型で表現不能にする — 型で潰せた分は runtime test が不要になります。詳細 → [`type-discipline.md`](type-discipline.md)。
>
> TS は `strict` + `any`/`as`/non-null `!` を hard gate (方針確定、閾値数値は pilot/別レビュー)、型表現力の弱い言語は advisory (L4 Mutation の tier 構造と同型)。
...
> - **L5.5 Visual Regression (VRT)** は **本 skill では採用せず**: pixel-diff oracle は semantic bug 検出に ceiling があるため。**visual 崩れの prevention 側** は `design/phases-4-6.md` の Phase 6.5 self-review (素人視点 self-check) が担う。L5 Smoke の実 DOM 検証とは目的が異なる (崩れ視認 vs journey 成立) ため併存・補完関係。
