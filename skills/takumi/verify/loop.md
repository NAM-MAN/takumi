...
4. kill されなければ、または mutant が **仕様違反の出力を示す**場合: **バグ確定扱いで即修正**
   - 軍師 (codex exec、GPT-5.x; 例示は default gpt-5.5、env.yaml の preference で固定可、`tk_timeout 600` + stdin heredoc / hang 時 subagent Tier 2 fallback、詳細: `gunshi-invocation.md`「GPT-5.5 upgrade path」+「invocation hardening v2」) に 400 字以内で相談: (a) 修正方針 (b) 既存テストへの影響 (c) 他の類似箇所波及
