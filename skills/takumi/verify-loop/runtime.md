...
詳細 recipe は **`../verify/compression.md`**。
...
### Phase 3.5 (optional): LLM property 自動提案 (C-stack)

**起動条件 (opt-in)**: codex CLI が使える環境で、軍師ルートに対応する project のみ。default では起動しない (`[[codex_long_prompt_hang_issue]]` のとおり 0.125.0 は長 prompt で hang し得るため、tick 単位で hang 率を監視)。

**手順**:

1. Phase 2 末で `reports/mutation/mutation.json` から生存 mutant top-N を **file 多様性順** にサンプル (1 file につき mutator 種別ごとに 1 件、N=3-5 推奨)。各 mutant に file/line/mutatorName と周辺 ~5 行の AST context を付けて記述
2. 1.5KB 以内の prompt を codex 5.5 に投げる:
   ```bash
   ( codex exec -m gpt-5.5 -s read-only --skip-git-repo-check -C "$(pwd)" - <<EOF > out.log 2>&1
   {1.5KB 以内の prompt: 生存 mutant 情報 + 周辺コード + 「vitest テストを 1 つ書け」}
   EOF
   ) & CPID=$!
   ( sleep 60 && kill -TERM $CPID 2>/dev/null && sleep 3 && kill -KILL $CPID 2>/dev/null ) &
   wait $CPID
   ```
3. 60s timeout、hang 検知時は **Sonnet fallback** (Agent tool subagent_type=general-purpose に同じ prompt)
4. 生成 property を test ファイルに append し、Stryker incremental で kill 確認
5. kill されなかった候補は `discovered-{id}.md` に「LLM 生成失敗」flag 付きで記録 → 人間レビュー

**C2 boundary mining**: 生存 `EqualityOperator` / `ConditionalExpression` mutant の context から閾値定数を抽出し、fast-check の `fc.constantFrom([thresh, thresh-1, thresh+1])` で arbitrary を補強。

**RETREAT 条件**: 1 tick 内で hang_rate ≥ 40% なら C-stack を縮退 (Sonnet fallback も hang した場合は Phase 3.5 を skip)。

**期待効果**: SHARPEN/ADD で人手不要、生存 mutant あたり 20-30s の codex 呼び出しコストと引き換えに 1-2 個の property を自動追加できる見込み (実 repo 計測必須)。

## 本体ドキュメント

詳細な手順・状態ファイル形式・Phase 定義は **`../verify/loop.md`** に集約。
...
Agent 内で実行される (Main では実行しない)。詳細は `../verify/loop.md`:
...
関連スキル:
- `../verify/README.md` — 検証全体方針
- `../verify/mutation.md` — Stryker 設定詳細
- `../sweep/README.md` — 同系統のループ対応オーケストレーター (参考)
