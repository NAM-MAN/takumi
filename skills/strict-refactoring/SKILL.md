...
description: "コード修正・リファクタリング・設計相談を受けた際の policy skill。/plan が refactor_profile_ref と strictness を選定して内部呼出、設計制約・UI state 契約・verify 接続を担う。単独起動も可 (「リファクタして」「設計見直して」等)。"
---

# Strict Refactoring: 設計制約 policy skill

`/plan` が task の `refactor_profile_ref` と `strictness` を frontmatter に書き、実装 worker が完了時に本 skill の checklist で再評価する二段構えで運用する。単独起動(自然文トリガー)も許容するが、その場合も同じ profile schema を仮生成する。
...
- `/plan` が task 生成時に **layer / code age / project_mode** から自動判定して内部呼出
...
- **/plan 経由**: task frontmatter に profile / strictness / tier が書かれている → 読み込んで適用
...
詳細 schema は `~/.claude/skills/plan/telemetry-spec.md` と `telemetry-schema.md` (後日追加)。
...
| `~/.claude/skills/plan/SKILL.md` | 呼出元 (refactor_profile_ref を書く) |
| `~/.claude/skills/verify/SKILL.md` | L1-L6 テスト戦略、Tier B の precondition 再利用 |
| `~/.claude/skills/design/SKILL.md` | 創発寄り、本 skill は制約寄り (対) |
