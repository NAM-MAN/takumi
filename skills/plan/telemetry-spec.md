...
| Emitter | タイミング | event |
|---------|-----------|-------|
| `/plan` | task 作成確定時 | `task_created` |
| `/plan` | profile override 時 | `profile_overridden` |
...
| # | event | 1 行説明 |
|---|-------|---------|
| 3.1 | `task_created` | /plan で task 確定時。profile_ref 付帯率の原資料 |
...
| 4+ | `RITUALIZATION DRIFT:` | 設計見直し推奨、/plan の default profile 再選定を提案 |
...
| # | 指標 | 一行説明 |
|---|------|---------|
| 5.1 | task 作成時間 | /plan interview の所要時間。機械的選定 / bypass 兆候の検出 |
...
| `SKILL.md` | /plan skill 本体。telemetry emit 組み込みポイント |
